...
async def add_user(user_id, user_name, full_name, room):
    connect = sqlite3.connect('booking.db')
    cursor = connect.cursor()
    cursor.execute('SELECT MAX(number) FROM users;')
    num = cursor.fetchone()[0] + 1
    m = [num, user_id, user_name, full_name, room]
    cursor.execute('INSERT INTO users (number, user_id, user_name, full_name, room) VALUES (?,?,?,?,?)', m)
    connect.commit()
    cursor.close()
...
async def get_room(user_id):
    connect = sqlite3.connect('booking.db')
    cursor = connect.cursor()
    name = cursor.execute(f"SELECT room FROM users WHERE user_id = '{user_id}'").fetchone()[0]
    connect.commit()
    cursor.close()
    return name
